...
namespace Base.AttributePackage.Editor
{
    ...
    public sealed class MinMaxHandler : IAfterFieldHandler
    {
        ...
        private static Vector3 Clamp(Vector3 value, float min, float max)
            => new(Mathf.Clamp(value.x, min, max), Mathf.Clamp(value.y, min, max), Mathf.Clamp(value.z, min, max));

        private static Vector2Int Clamp(Vector2Int value, int min, int max)
            => new(Mathf.Clamp(value.x, min, max), Mathf.Clamp(value.y, min, max));

        private static Vector3Int Clamp(Vector3Int value, int min, int max)
            => new(Mathf.Clamp(value.x, min, max), Mathf.Clamp(value.y, min, max), Mathf.Clamp(value.z, min, max));
    }
}
