...
namespace Base.AttributePackage.Editor
{
    ...
    public sealed class MaxHandler : IAfterFieldHandler
    {
        ...
        private static Vector3 ClampMax(Vector3 value, float max) => new(Mathf.Min(value.x, max),
            Mathf.Min(value.y, max), Mathf.Min(value.z, max));

        private static Vector2Int ClampMax(Vector2Int value, int max)
            => new(Mathf.Min(value.x, max), Mathf.Min(value.y, max));

        private static Vector3Int ClampMax(Vector3Int value, int max) => new(Mathf.Min(value.x, max),
            Mathf.Min(value.y, max), Mathf.Min(value.z, max));
    }
}